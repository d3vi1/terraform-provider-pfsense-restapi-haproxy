...
| `pfsense_haproxy_backend_acl` | HAProxyBackendACL | `GET/POST/PATCH/DELETE /api/v2/services/haproxy/backend/acl`; `GET /backend/acls` | Create requires `parent_id`, `name`, `expression`, `value`; ordered arrays accept `placement`. Patch requires `parent_id`, `id`. | Implemented in M5-01 as an ordered backend child resource using `backend_name/acl_name` as Terraform's stable natural key. Before writes, resolve the parent backend by name; before update/delete, resolve the current ACL child ID by exact ACL name. Expose `position` as zero-based order and send pfREST `placement` on create/update when configured or changed. Duplicate ACL names under one backend are diagnostics. |
| `pfsense_haproxy_backend_action` | HAProxyBackendAction | `GET/POST/PATCH/DELETE /api/v2/services/haproxy/backend/action`; `GET /backend/actions` | Create requires `parent_id` plus action-specific fields; ordered arrays accept `placement`. Patch requires `parent_id`, `id`. | Implemented in M5-01 as an ordered backend child resource using `backend_name/key` as Terraform's stable ID. The `key` is Terraform-only and is never sent to pfSense. Because backend actions have no stable name, live children are matched by normalized action payload fields excluding `id`, `parent_id`, `position`, and `key`; duplicate payload matches are diagnostics. Conditional fields are validated from the pfREST model and irrelevant non-null fields are rejected to keep identity unambiguous. |
...
| `pfsense_haproxy_frontend_acl` | HAProxyFrontendACL | `GET/POST/PATCH/DELETE /api/v2/services/haproxy/frontend/acl`; `GET /frontend/acls` | Create requires `parent_id`, `name`, `expression`, `value`; ordered arrays accept `placement`. Patch requires `parent_id`, `id`. | Implemented in M5-02 as an ordered frontend child resource using `frontend_name/acl_name` as Terraform's stable natural key. Before writes, resolve the parent frontend by name; before update/delete, resolve the current ACL child ID by exact ACL name. Expose `position` as zero-based order and send pfREST `placement` on create/update when configured or changed. Duplicate ACL names under one frontend are diagnostics. |
| `pfsense_haproxy_frontend_action` | HAProxyFrontendAction | `GET/POST/PATCH/DELETE /api/v2/services/haproxy/frontend/action`; `GET /frontend/actions` | Create requires `parent_id` plus action-specific fields; ordered arrays accept `placement`. Patch requires `parent_id`, `id`. | Implemented in M5-02 as an ordered frontend child resource using `frontend_name/key` as Terraform's stable ID. The `key` is Terraform-only and is never sent to pfSense. Because frontend actions have no stable name, live children are matched by normalized action payload fields excluding `id`, `parent_id`, `position`, and `key`; duplicate payload matches are diagnostics. Conditional fields are validated from the pfREST model and irrelevant non-null fields are rejected to keep identity unambiguous. Frontend routing uses action `use_backend` with the conditional `backend` field. |
...
- Confirm `GET /api/v2/services/haproxy/frontend/acls?parent_id=...&name=...`
  returns exact frontend ACL matches or, at minimum, a list containing `id`,
  `name`, `expression`, and `value` fields that can be filtered client-side.
- Confirm frontend ACL full plural reads preserve child order for
  `position`/`placement` management.
- Confirm `GET /api/v2/services/haproxy/frontend/actions?parent_id=...`
  returns ordered frontend action objects with transient `id`, `action`, `acl`,
  and action-specific fields, including dynamic action-prefixed internal names
  such as `http-request_set-headername` and `lua-function`.
- Confirm frontend action writes use `use_backend` plus `backend` for route
  selection and only mark HAProxy changes pending until explicit apply.
