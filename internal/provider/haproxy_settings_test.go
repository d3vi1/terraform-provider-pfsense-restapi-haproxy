...
func TestProviderRegistersHaproxySettings(t *testing.T) {
	...
	if !resourceTypeRegistered(resources, "pfsense_haproxy_backend_acl") {
		t.Fatalf("pfsense_haproxy_backend_acl resource was not registered")
	}
	if !resourceTypeRegistered(resources, "pfsense_haproxy_backend_action") {
		t.Fatalf("pfsense_haproxy_backend_action resource was not registered")
	}
	...
	if !resourceTypeRegistered(resources, "pfsense_haproxy_frontend_acl") {
		t.Fatalf("pfsense_haproxy_frontend_acl resource was not registered")
	}
	if !resourceTypeRegistered(resources, "pfsense_haproxy_frontend_action") {
		t.Fatalf("pfsense_haproxy_frontend_action resource was not registered")
	}
