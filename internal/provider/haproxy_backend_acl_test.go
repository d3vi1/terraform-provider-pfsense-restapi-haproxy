...
func TestHaproxyBackendACLResourceCreateUsesParentLookupPositionAndDoesNotApply(t *testing.T) {
	t.Parallel()
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case http.MethodGet + " /api/v2/services/haproxy/backend/acls":
		...
		case http.MethodPost + " /api/v2/services/haproxy/backend/acl":
	...
	aclResource := &haproxyBackendACLResource{
		client: pfsense.NewClient(pfsense.Config{Endpoint: server.URL, APIKey: "test-key"}),
	}
	schema := haproxyBackendACLResourceSchema(t)
	plan := nullHaproxyBackendACLModel()
	...
	wantRequests := []string{
		...
		"GET /api/v2/services/haproxy/backend/acls?name=host_acl&parent_id=42",
		"POST /api/v2/services/haproxy/backend/acl",
		"GET /api/v2/services/haproxy/backend/acls?name=host_acl&parent_id=42",
		"GET /api/v2/services/haproxy/backend/acls?parent_id=42",
	}
	...
	if state.ID.ValueString() != "app_backend/host_acl" || state.Position.ValueInt64() != 0 || !state.CaseSensitive.ValueBool() {
		t.Fatalf("state was not refreshed from API: %#v", state)
	}
}

func TestHaproxyBackendACLResourceReadUsesParentLookupAndOrderedPosition(t *testing.T) {
	t.Parallel()
	...
	schema := haproxyBackendACLResourceSchema(t)
	stateModel := nullHaproxyBackendACLModel()
	stateModel.ID = types.StringValue("app_backend/host_acl")
...
func TestHaproxyBackendACLResourceUpdatePatchesPlacementAndChangedFields(t *testing.T) {
	t.Parallel()
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case http.MethodPatch + " /api/v2/services/haproxy/backend/acl":
	...
	schema := haproxyBackendACLResourceSchema(t)
	prior := nullHaproxyBackendACLModel()
	prior.ID = types.StringValue("app_backend/host_acl")
	...
	for _, forbidden := range []string{"name", "backend_name", "apply", "async", "key"} {
...
func TestHaproxyBackendACLResourceDeleteResolvesCurrentParentAndChildIDs(t *testing.T) {
	t.Parallel()
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case http.MethodGet + " /api/v2/services/haproxy/backend/acls":
			...
			_, _ = w.Write([]byte(`{"code":200,"status":"ok","data":[{"id":77,"name":"host_acl","expression":"host_matches","value":"app.example.com"}]}`))
		case http.MethodDelete + " /api/v2/services/haproxy/backend/acl":
	...
	wantRequests := []string{
		"GET /api/v2/services/haproxy/backends?name=app_backend",
		"GET /api/v2/services/haproxy/backend/acls?name=host_acl&parent_id=99",
		"GET /api/v2/services/haproxy/backend/acls?parent_id=99",
		"DELETE /api/v2/services/haproxy/backend/acl?id=77&parent_id=99",
	}
	if !reflect.DeepEqual(gotRequests, wantRequests) {
		t.Fatalf("requests = %#v, want %#v", gotRequests, wantRequests)
	}
...
func TestHaproxyBackendACLResourceDuplicateNamesDiagnostic(t *testing.T) {
	t.Parallel()
	...
	if !strings.Contains(diagnosticsText(resp.Diagnostics), "multiple HAProxy backend ACLs named") {
		t.Fatalf("diagnostics did not describe duplicate ACLs: %s", diagnosticsText(resp.Diagnostics))
	}
}

func TestHaproxyBackendACLResourceImportUsesBackendAndACLNames(t *testing.T) {
	t.Parallel()
	...
	aclResource.ImportState(context.Background(), resource.ImportStateRequest{ID: "app_backend/host_acl"}, &validResp)
	...
	if state.ID.ValueString() != "app_backend/host_acl" || state.BackendName.ValueString() != "app_backend" || state.Name.ValueString() != "host_acl" {
		t.Fatalf("imported state = %#v", state)
	}

	for _, id := range []string{"", "app_backend", "app_backend/", "/host_acl", "app_backend/host_acl/extra", "app_backend/a"} {
...
func haproxyBackendACLResourceSchema(t *testing.T) resourceschema.Schema {
	t.Helper()

	var resp resource.SchemaResponse
	(&haproxyBackendACLResource{}).Schema(context.Background(), resource.SchemaRequest{}, &resp)
	if resp.Diagnostics.HasError() {
